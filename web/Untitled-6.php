<?php
	...
	if($_POST['enviar']==""){
		...
		$fatura=$consumo->litrosMes($ultimo,$mysqli);
		}
	}else{
		$meses=$consumo->meses($mysqli);
		if($_POST['mes']!="" && $_POST['inicio']=="" && $_POST['fim']==""){
			$ultimo=$_POST['mes'];
			$fatura=$consumo->litrosMes($_POST['mes'],$mysqli);
		}elseif($_POST['inicio']!="" || $_POST['fim']!=""){
			$fatura=$consumo->litrosPeriodo($_POST['inicio'],$_POST['fim'],$mysqli);
			var_dump($fatura);
			$ultimo=$_POST['mes'];
			//echo "oi";
		}	
...
?>	
	...
	<div class="col-md-6 txt1" style="width:1200px;">
        ...
        <h3 STYLE="text-align-last:left;text-transform: none;"><?=$fatura?> m³</h3></br>
        <h3 STYLE="text-align-last:left;text-transform: none;"><?=$fatura?>000 litros consumidos  </h3>
        <h3 STYLE="text-align-last:left;text-transform: none;">(aproximadamente) </h3>
        
        <p style="color:#000;text-transform: none;text-align: right;">Fatura de água do mês de <? echo mesExtenso($_POST['mes'])?></p>
		...
		<p style="color:#000;text-transform: none;text-align: right;"><a href="grafico_litros.php?mes=<?=$mes?>&inicio=<?=$_POST['inicio']?>&fim=<?=$_POST['fim']?>">Gráfico</a></p>
